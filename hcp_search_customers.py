...
def get_inputs() -> dict:
    ...
    parser.add_argument("--api-key", metavar="KEY",     help="HCP API key (overrides env var)")
...
def resolve_api_key(inputs: dict) -> str:
    ...
    """
    key = (inputs.get("api_key") or "").strip() or os.environ.get("HCP_API_KEY", "").strip()
...
def format_customer(match: dict) -> str:
    ...
    matched_str = (
        ...
        if matches else "(returned by API query)"
    )
...
if __name__ == "__main__":
    ...
    if not any(inputs.get(f) for f in ["name", "email", "phone", "address"]):
        print("❌  Provide at least one of --name, --email, --phone, --address", file=sys.stderr)
        sys.exit(1)
